...
void shift_OGvalue_by_10_add_NEWvalue(short NEWvalue){
    if (typedCode >= 100000){
        typedCode = 0;
    }else{
        typedCode = typedCode * 10 + NEWvalue;
    }
    sprintf(strCode, "%d", typedCode);
    lv_label_set_text(ui_ASLabelEnteredPasscode,strCode);
    lv_label_set_text(ui_AALabelEnteredPasscode,strCode);
}
void testCheckTypedCode(short screenOnSuccess){
    if (typedCode == 1212){
         typedCode = 0;
         if (screenOnSuccess == 1){
            //lv_obj_clean(lv_scr_act());
            //lv_obj_t * screen;
            //screen = lv_obj_create(NULL, NULL);
            lv_scr_load(ui_AlarmArmed);//screen);
            lv_indev_wait_release(lv_indev_get_act());
            //lv_obj_t * cont = lv_cont_create(ui_AlarmArmed, NULL);
    
            lv_label_set_text(ui_AALabelEnteredPasscode,strCode);
         
            //_ui_screen_change(ui_AlarmArmed, LV_SCR_LOAD_ANIM_OVER_LEFT,1,1,false);
         }else{
            //lv_obj_clean(lv_scr_act());
            //lv_obj_t * screen;
            //screen = lv_obj_create(NULL, NULL);
            ...
            lv_label_set_text(ui_ASLabelEnteredPasscode,strCode);
            //_ui_screen_change(ui_AlarmSet, LV_SCR_LOAD_ANIM_OVER_LEFT,1,1,false);
